...
public final class TransactionBuilder {
    ...
    func buildTransaction(from transaction: NormalTransaction) -> AnyPublisher<TransactionInstance?, Never> {
        guard let from = Go23Wallet.Address(string: transaction.from) else {
            return .just(nil)
        }
        ...
        let to = Go23Wallet.Address(string: transaction.to)?.eip55String ?? transaction.to
    ...
    private func fetchLocalizedOperation(contract: Go23Wallet.Address) -> AnyPublisher<TransactionBuilder.LocalizedOperation, SessionTaskError> {
