...
public final class TokensFromTransactionsFetcher {
    ...
    private func addTokensFromUpdates(transactionsToPullContractsFrom transactions: [TransactionInstance], contractsAndTokenTypes: [DerbyWallet.Address: TokenType]) {
    ...
    private var contractsToAvoid: [DerbyWallet.Address] {
    ...
    private func filterTransactionsToPullContractsFrom(_ transactions: [TransactionInstance]) -> Promise<(transactions: [TransactionInstance], contractTypes: [DerbyWallet.Address: TokenType])> {
        let contractsToAvoid = contractsToAvoid
        let filteredTransactions = transactions.filter {
            if let toAddressToCheck = DerbyWallet.Address(string: $0.to), contractsToAvoid.contains(toAddressToCheck) {
                return false
            }
...
extension TokensFromTransactionsFetcher.Functional {
    ...
    static func tokens(from transactions: [TransactionInstance], contractsAndTokenTypes: [DerbyWallet.Address: TokenType]) -> [TokenUpdate] {
        let tokens: [TokenUpdate] = transactions.flatMap { transaction -> [TokenUpdate] in
            let tokenUpdates: [TokenUpdate] = transaction.localizedOperations.compactMap { operation in
                ...
                if let type = contractsAndTokenTypes[contract] {
                    tokenType = type
                } else {
