        ...
        private func covalentTxToNativeTx(tx: Covalent.Transaction, server: RPCServer) -> TransactionInstance? {
            ...
            let operations: [LocalizedOperationObjectInstance] = tx
                ...
                .compactMap { logEvent -> LocalizedOperationObjectInstance? in
                    guard let contractAddress = DerbyWallet.Address(uncheckedAgainstNullAddress: logEvent.senderAddress) else { return nil }
                    var params = logEvent.params
                    //TODO: Improve with adding more transaction types, approve and other
                    guard let from = DerbyWallet.Address(uncheckedAgainstNullAddress: params["from"]?.value ?? "") else { return nil }
                    guard let to = DerbyWallet.Address(uncheckedAgainstNullAddress: params["to"]?.value ?? "") else { return nil }
