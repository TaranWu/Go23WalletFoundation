...
extension Covalent {
    ...
    struct BalancesResponse {
        ...
        init(json: JSON) throws {
            guard
                let address = Go23Wallet.Address(string: json["address"].stringValue),
                let chainId = json["chain_id"].int,
                let quoteCurrency = json["quote_currency"].string,
                let items = json["items"].array else {
                throw DecodyingError.fieldMismatch
        ...
        struct NFTAsset {
            ...
            let originalOwner: Go23Wallet.Address
            let owner: Go23Wallet.Address

            init(json: JSON) throws {
                guard
                    let originalOwner = Go23Wallet.Address(string: json["original_owner"].stringValue),
                    let owner = Go23Wallet.Address(string: json["owner_address"].stringValue),
        ...
        struct Token {
            ...
            init(json: JSON) throws {
                guard
                    let address = Go23Wallet.Address(string: json["contract_address"].stringValue),
