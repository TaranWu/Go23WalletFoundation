//
//  Erc1155TransferTransactionSchedulerProvider.swift
...
protocol Erc1155TransferTransactionSchedulerDelegate: AnyObject {
...
final class Erc1155TransferTransactionSchedulerProvider: SchedulerProvider {
    ...
    var name: String { "erc1155TransferTransactionSchedulerProvider" }
    var operation: AnyPublisher<Void, PromiseError> {
        return fetchPublisher()
    }

    weak var delegate: Erc1155TransferTransactionSchedulerDelegate?
    ...
    private func fetchPublisher() -> AnyPublisher<Void, PromiseError> {
        let pagination = storage
            .transactionsPagination(server: session.server, fetchType: .erc1155) ?? defaultPagination

        return networking
            .erc1155TokenTransferTransaction(walletAddress: session.account.address, pagination: pagination, sortOrder: nil)
