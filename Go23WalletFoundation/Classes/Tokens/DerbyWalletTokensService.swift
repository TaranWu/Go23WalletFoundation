//
//  DerbyWalletTokensService.swift
//  DerbyWallet
//
...
public class DerbyWalletTokensService: TokensService {
    ...
    private let queue = DispatchQueue(label: "org.DerbyWallet.swift.tokensService", qos: .utility)
    ...
    public lazy var tokensPublisher: AnyPublisher<[Token], Never> = {
        providers.map { $0.values }
            ...
            }.map { DerbyWalletTokensService.filterAwaySpuriousTokens($0) }
            .eraseToAnyPublisher()
    }()
    ...
    public func token(for contract: DerbyWallet.Address) -> Token? {
        return tokensDataStore.token(forContract: contract)
    }

    public func token(for contract: DerbyWallet.Address, server: RPCServer) -> Token? {
    ...
    public func tokenPublisher(for contract: DerbyWallet.Address, server: RPCServer) -> AnyPublisher<Token?, Never> {
...
extension DerbyWalletTokensService: TokensServiceTests {
...
extension DerbyWalletTokensService {
