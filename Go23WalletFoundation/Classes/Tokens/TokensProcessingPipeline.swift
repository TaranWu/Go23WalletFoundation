...
public protocol TokenViewModelState {
    ...
    func tokenViewModelPublisher(for contract: Go23Wallet.Address, server: RPCServer) -> AnyPublisher<TokenViewModel?, Never>
    func tokenViewModel(for contract: Go23Wallet.Address, server: RPCServer) -> TokenViewModel?
}
...
public final class WalletDataProcessingPipeline: TokensProcessingPipeline {
    ...
    private let queue = DispatchQueue(label: "org.Go23Wallet.swift.walletData.processingPipeline", qos: .userInitiated)
    ...
    public func tokenViewModel(for contract: Go23Wallet.Address, server: RPCServer) -> TokenViewModel? {
    ...
    public func tokenViewModelPublisher(for contract: Go23Wallet.Address, server: RPCServer) -> AnyPublisher<TokenViewModel?, Never> {
    ...
    public func token(for contract: Go23Wallet.Address) -> Token? {
        tokensService.token(for: contract)
    }

    public func token(for contract: Go23Wallet.Address, server: RPCServer) -> Token? {
    ...
    public func tokenPublisher(for contract: Go23Wallet.Address, server: RPCServer) -> AnyPublisher<Token?, Never> {
