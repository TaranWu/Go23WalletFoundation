...
struct UniqueNonEmptyContractsDecoder {
    func decode(transactions: [NormalTransaction]) -> UniqueNonEmptyContracts {
        ...
        let uniqueNonEmptyContracts = Set(nonEmptyContracts).compactMap { Go23Wallet.Address(uncheckedAgainstNullAddress: $0) }
...
extension UniqueNonEmptyContracts {
    init(json: JSON) {
        ...
        uniqueNonEmptyContracts = Set(nonEmptyContracts).compactMap { Go23Wallet.Address(uncheckedAgainstNullAddress: $0) }
