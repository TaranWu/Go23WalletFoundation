...
public protocol TokenFetcher: NSObjectProtocol {
    func fetchTokenOrContract(for contract: DerbyWallet.Address, onlyIfThereIsABalance: Bool) -> Promise<TokenOrContract>
}
...
public class SingleChainTokenFetcher: NSObject, TokenFetcher {
    ...
    private var inflightPromises: AtomicDictionary<DerbyWallet.Address, Promise<TokenOrContract>> = .init()
    ...
    public func fetchTokenOrContract(for contract: DerbyWallet.Address, onlyIfThereIsABalance: Bool = false) -> Promise<TokenOrContract> {
