...
public class GetContractInteractions {
    ...
    public func getErc20Interactions(walletAddress: DerbyWallet.Address, server: RPCServer, startBlock: Int? = nil) -> Promise<[TransactionInstance]> {
        return Functional.getErc20Interactions(walletAddress: walletAddress, server: server, startBlock: startBlock, queue: queue)
    }

    public func getErc721Interactions(walletAddress: DerbyWallet.Address, server: RPCServer, startBlock: Int? = nil) -> Promise<[TransactionInstance]> {
        return Functional.getErc721Interactions(walletAddress: walletAddress, server: server, startBlock: startBlock, queue: queue)
    }

    public func getContractList(walletAddress: DerbyWallet.Address, server: RPCServer, startBlock: Int? = nil, erc20: Bool) -> Promise<([DerbyWallet.Address], Int?)> {
...
extension GetContractInteractions.Functional {
    ...
    static func getErc20Interactions(walletAddress: DerbyWallet.Address, server: RPCServer, startBlock: Int? = nil, queue: DispatchQueue) -> Promise<[TransactionInstance]> {
        ...
        return firstly {
            ...
            let transactions: [TransactionInstance] = filteredResult.map { result in
                ...
                let localizedTokenObj = LocalizedOperationObjectInstance(
                        ...
                        contract: DerbyWallet.Address(uncheckedAgainstNullAddress: transactionJson["contractAddress"].stringValue),
    ...
    static func getErc721Interactions(walletAddress: DerbyWallet.Address, server: RPCServer, startBlock: Int? = nil, queue: DispatchQueue) -> Promise<[TransactionInstance]> {
    ...
    static func getContractList(walletAddress: DerbyWallet.Address, server: RPCServer, startBlock: Int? = nil, erc20: Bool, queue: DispatchQueue) -> Promise<([DerbyWallet.Address], Int?)> {
        ...
        return firstly {
            ...
            let nonEmptyContracts = contracts
                    .map { $0.0 }
                    .filter { !$0.isEmpty }
            let uniqueNonEmptyContracts = Set(nonEmptyContracts).compactMap { DerbyWallet.Address(uncheckedAgainstNullAddress: $0) }
