...
public class Erc1155TokenIdsFetcher {
    ...
    private func knownErc1155Contracts() -> Set<Go23Wallet.Address>? {
        guard let contractsAndTokenIds = readJson() else { return nil }
        return Set(contractsAndTokenIds.tokens.keys)
    }

    public func filterAwayErc1155Tokens(contracts: [Go23Wallet.Address]) -> [Go23Wallet.Address] {
    ...
    static private func fileUrl(forWallet address: Go23Wallet.Address, server: RPCServer) -> URL {
    ...
    static private func fileUrlV1(forWallet address: Go23Wallet.Address, server: RPCServer) -> URL {
    ...
    public static func deleteForWallet(_ address: Go23Wallet.Address) {
    ...
    fileprivate func fetchEvents(transferType: Erc1155TransferEvent.TransferType, eventName: String, parameterFilters: [[EventFilterable]?], fromBlock: EventFilter.Block, toBlock: EventFilter.Block) -> AnyPublisher<[Erc1155TransferEvent], Erc1155TokenIdsFetcherError> {
        ...
        return blockchainProvider
            .eventLogs(contractAddress: dummyContract, eventName: eventName, abiString: Go23Wallet.Ethereum.ABI.erc1155String, filter: eventFilter)
            .map { events -> [Erc1155TransferEvent] in
                ...
                let results: [Erc1155TransferEvent] = sortedEvents.flatMap { each -> [Erc1155TransferEvent] in
                    let contract = Go23Wallet.Address(address: each.eventLog!.address)
                    guard let from = ((each.decodedResult["_from"] as? EthereumAddress).flatMap({ Go23Wallet.Address(address: $0) })) else { return [] }
                    guard let to = ((each.decodedResult["_to"] as? EthereumAddress).flatMap({ Go23Wallet.Address(address: $0) })) else { return [] }
