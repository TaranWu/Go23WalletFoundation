...
public struct Erc1155TokenIds: Codable {
    public typealias ContractsAndTokenIds = [DerbyWallet.Address: Set<BigUInt>]
...
public class Erc1155TokenIdsFetcher {
    ...
    public init(address: DerbyWallet.Address, server: RPCServer, config: Config, queue: DispatchQueue) {
    ...
    public func knownErc1155Contracts() -> Set<DerbyWallet.Address>? {
    ...
    static private func fileUrl(forWallet address: DerbyWallet.Address, server: RPCServer) -> URL {
    ...
    private static func writeJson(contractsAndTokenIds: Erc1155TokenIds, address: DerbyWallet.Address, server: RPCServer) -> Promise<Void> {
        Promise { seal in
    ...
    public static func deleteForWallet(_ address: DerbyWallet.Address) {
...
extension Erc1155TokenIdsFetcher.Functional {
    ...
    static func fetchEvents(config: Config, forAddress address: DerbyWallet.Address, server: RPCServer, fromBlock: Web3.EventFilter.Block, toBlock: Web3.EventFilter.Block, queue: DispatchQueue) -> Promise<Erc1155TokenIds> {
    ...
    fileprivate static func fetchEvents(config: Config, server: RPCServer, transferType: Erc1155TransferEvent.TransferType, eventName: String, parameterFilters: [[Web3.EventFilterable]?], fromBlock: Web3.EventFilter.Block, toBlock: Web3.EventFilter.Block, queue: DispatchQueue) -> Promise<[Erc1155TransferEvent]> {
        ...
        return firstly {
            getEventLogs(withServer: server, contract: dummyContract, eventName: eventName, abiString: DerbyWallet.Ethereum.ABI.erc1155String, filter: eventFilter, queue: queue)
            ...
            let results: [Erc1155TransferEvent] = sortedEvents.flatMap { each -> [Erc1155TransferEvent] in
                let contract = DerbyWallet.Address(address: each.eventLog!.address)
                guard let from = ((each.decodedResult["_from"] as? Web3.EthereumAddress).flatMap({ DerbyWallet.Address(address: $0) })) else { return [] }
                guard let to = ((each.decodedResult["_to"] as? Web3.EthereumAddress).flatMap({ DerbyWallet.Address(address: $0) })) else { return [] }
