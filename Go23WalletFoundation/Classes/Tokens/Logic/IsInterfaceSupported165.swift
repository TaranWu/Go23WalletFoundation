...
public class IsInterfaceSupported165 {
    ...
    private let queue = DispatchQueue(label: "org.Go23Wallet.swift.isInterfaceSupported165")
    ...
    public func getInterfaceSupported165(hash: String, contract: Go23Wallet.Address) -> AnyPublisher<Bool, SessionTaskError> {
        return Just(hash)
            .receive(on: queue)
