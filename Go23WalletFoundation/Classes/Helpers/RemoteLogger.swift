...
func logError(_ err: Error, pref: String = "", function funcName: String = #function, rpcServer: RPCServer? = nil, address: DerbyWallet.Address? = nil) {
    var description = pref
    description += rpcServer.flatMap { " server: \($0)" } ?? ""
    description += address.flatMap { " address: \($0.eip55String)" } ?? ""
    description += " \(err)"
    warnLog(description, callerFunctionName: funcName)
}
