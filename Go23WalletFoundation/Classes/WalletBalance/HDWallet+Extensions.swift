...
extension HDWallet {
    ...
    public static let validSeedPhraseCounts = [HDWallet.SeedPhraseCount.word12.count, HDWallet.SeedPhraseCount.word24.count]
    ...
    public static func leftPadStringWithZero(_ string: String, to count: Int) -> String {
    ...
    public static func computeSeedWithChecksum(fromSeedPhrase seedPhrase: String) -> String {
        let seedPhraseArray = seedPhrase.split(separator: " ").map { String($0) }
        let indices = seedPhraseArray.compactMap { HDWallet.englishWordList.firstIndex(of: $0) }
        //11-bits index for each wor the seed phrase
        let binaryIndices = indices.map { leftPadStringWithZero(String($0, radix: 2), to: 11) }
        let inBase2 = BigInt(binaryIndices.joined(), radix: 2)!
        return String(inBase2, radix: 16)
    }

    public convenience init?(seed: String, passphrase: String) {
