...
open class TokenSwapper: ObservableObject {
    ...
    public func supports(contractAddress: DerbyWallet.Address, server: RPCServer) -> Bool {
    ...
    public func fetchSwapQuote(fromToken: TokenToSwap, toToken: TokenToSwap, wallet: DerbyWallet.Address, slippage: String, fromAmount: BigUInt) -> AnyPublisher<Result<SwapQuote, SwapError>, Never> {
        return networkProvider.fetchSwapQuote(fromToken: fromToken, toToken: toToken, wallet: wallet, slippage: slippage, fromAmount: fromAmount)
            ...
            .catch { err -> AnyPublisher<Result<SwapQuote, SwapError>, Never> in
