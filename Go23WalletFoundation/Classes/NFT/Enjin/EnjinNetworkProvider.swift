...
final class EnjinNetworkProvider {
    ...
    typealias GetEnjinBalancesResponse = (balances: Enjin.MappedEnjinBalances, owner: DerbyWallet.Address)

    func getEnjinBalances(forOwner owner: DerbyWallet.Address, offset: Int, sum: Enjin.MappedEnjinBalances = [:], limit: Int = 50, completion: @escaping (Swift.Result<GetEnjinBalancesResponse, EnjinError>) -> Void) {
    ...
    func getEnjinTokens(ids: [String], owner: DerbyWallet.Address) -> Promise<[DerbyWallet.Address: [GetEnjinTokenQuery.Data.EnjinToken]]> {
        if ids.isEmpty {
            return .value([owner: []])
        }
