...
public class TransactionConfigurator {
    ...
    public var toAddress: DerbyWallet.Address? {
...
// swiftlint:disable function_body_length
    private static func createConfiguration(server: RPCServer, analytics: AnalyticsLogger, transaction: UnconfirmedTransaction, account: DerbyWallet.Address) throws -> TransactionConfiguration {
        ...
        do {
            ...
            case .erc875TokenOrder(let token, _):
                guard let expiry = transaction.expiry, let indices = transaction.indices, let v = transaction.valuev, let r = transaction.valuer, let s = transaction.values else {
                    throw TransactionConfiguratorError.impossibleToBuildConfiguration
                }
                let parameters: [Any] = [
                    expiry,
