...
public class AddCustomChain {
    ...
    public func run() {
        firstly {
            ...
            Functional.checkAndDetectUrls(customChain, chainId: chainId, analytics: self.analytics, chainNameFallback: self.chainNameFallback).recover { error -> Promise<CustomChainWithChainIdAndRPC> in
                if case ResolveExplorerApiHostnameError.resolveExplorerApiHostnameFailure = error {
