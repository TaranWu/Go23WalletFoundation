...
public enum RPCServer: Hashable, CaseIterable {
    ...
    public static var customRpcs: [CustomRPC] = RPCServer.convertJsonToCustomRpcs(Config().customRpcServersJson) {
        didSet {
            if let data = try? JSONEncoder().encode(customRpcs), let json = String(data: data, encoding: .utf8) {
                var config = Config()
                config.customRpcServersJson = json
    ...
    var erc20AddressForNativeToken: DerbyWallet.Address? {
        switch self {
        case .optimistic, .optimisticKovan: return DerbyWallet.Address(string: "0x4200000000000000000000000000000000000006")!
    ...
    func getEtherscanURLForGeneralTransactionHistory(for address: DerbyWallet.Address, startBlock: Int?) -> URL? {
    ...
    func getEtherscanURLForTokenTransactionHistory(for address: DerbyWallet.Address, startBlock: Int?) -> URL? {
    ...
    func getEtherscanURLForERC721TransactionHistory(for address: DerbyWallet.Address, startBlock: Int?) -> URL? {
    ...
    public func etherscanContractDetailsWebPageURL(for address: DerbyWallet.Address) -> URL? {
        switch self {
    ...
    public func etherscanTokenDetailsWebPageURL(for address: DerbyWallet.Address) -> URL? {
        switch self {
    ...
    var priceID: DerbyWallet.Address {
        switch self {
        case .main, .ropsten, .rinkeby, .kovan, .sokol, .custom, .xDai, .phi, .goerli, .artis_sigma1, .artis_tau1, .binance_smart_chain, .binance_smart_chain_testnet, .heco, .heco_testnet, .fantom, .fantom_testnet, .avalanche, .avalanche_testnet, .candle, .polygon, .mumbai_testnet, .optimistic, .optimisticKovan, .cronosTestnet, .arbitrum, .arbitrumRinkeby, .palm, .palmTestnet, .klaytnCypress, .klaytnBaobabTestnet, .ioTeX, .ioTeXTestnet:
            return DerbyWallet.Address(string: "0x000000000000000000000000000000000000003c")!
        case .poa: return DerbyWallet.Address(string: "0x00000000000000000000000000000000000000AC")!
        case .classic: return DerbyWallet.Address(string: "0x000000000000000000000000000000000000003D")!
        case .callisto: return DerbyWallet.Address(string: "0x0000000000000000000000000000000000000334")!
        }
    }
