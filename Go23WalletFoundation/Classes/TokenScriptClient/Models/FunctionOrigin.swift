...
public struct FunctionOrigin {
    ...
    public func makeUnConfirmedTransaction(withTokenObject token: Token, tokenId: TokenId, attributeAndValues: [AttributeId: AssetInternalValue], localRefs: [AttributeId: AssetInternalValue], server: RPCServer, session: WalletSession) throws -> (UnconfirmedTransaction, DecodedFunctionCall) {
    ...
    public func generateDataAndValue(withTokenId tokenId: TokenId, attributeAndValues: [AttributeId: AssetInternalValue], localRefs: [AttributeId: AssetInternalValue], server: RPCServer, session: WalletSession, keystore: Keystore) -> (Data?, BigUInt)? {
        assert(functionType.isTransaction)
        let payload: Data?
        let value: BigUInt
        switch functionType {
