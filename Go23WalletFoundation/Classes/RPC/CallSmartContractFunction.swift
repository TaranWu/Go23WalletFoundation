...
public func callSmartContract(withServer server: RPCServer, contract contractAddress: Go23Wallet.Address, functionName: String, abiString: String, parameters: [AnyObject] = [], shouldDelayIfCached: Bool = false) -> Promise<[String: Any]> {
    firstly {
        .value(server)
        ...
        if let (cachedPromise, cacheTimestamp) = smartContractCallsCache[cacheKey], now.timeIntervalSince(cacheTimestamp) < ttlForCache {
            ...
            let promise: Promise<[String: Any]> = promiseCreator.callPromise(options: web3Options)
                .recover(on: callSmartContractQueue, { error -> Promise<[String: Any]> in
                    ...
                    guard case Go23Web3Swift.Web3Error.rateLimited = error else { throw error }
                    throw error
                })
...
public func getSmartContractCallData(withServer server: RPCServer, contract contractAddress: Go23Wallet.Address, functionName: String, abiString: String, parameters: [AnyObject] = []) -> Data? {
...
final class GetEventLogs {
    private let queue = DispatchQueue(label: "org.Go23Wallet.swift.eth.getEventLogs", qos: .utility)
    private var inFlightPromises: [String: Promise<[EventParserResultProtocol]>] = [:]

    func getEventLogs(contractAddress: Go23Wallet.Address, server: RPCServer, eventName: String, abiString: String, filter: EventFilter) -> Promise<[EventParserResultProtocol]> {
        firstly {
    ...
    //Exposed for testing
    static func generateEventLogCachingKey(contractAddress: Go23Wallet.Address, server: RPCServer, eventName: String, abiString: String, filter: EventFilter) -> String {
