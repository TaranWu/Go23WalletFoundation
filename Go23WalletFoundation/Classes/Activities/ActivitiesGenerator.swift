...
class ActivitiesGenerator {
    ...
    private func getTokensAndXmlHandlers(tokens: [Token]) -> TokenObjectsAndXMLHandlers {
        return tokens.compactMap { token -> (contract: Go23Wallet.Address, server: RPCServer, xmlHandler: XMLHandler)? in
    ...
    private func getActivities(contract: Go23Wallet.Address,
                               server: RPCServer,
    ...
    private func generateImplicitAttributesForToken(contract: Go23Wallet.Address, server: RPCServer, symbol: String) -> [String: AssetInternalValue] {
