...
public class ActivitiesService: NSObject, ActivitiesServiceType {
    private typealias ContractsAndCards = [(tokenContract: DerbyWallet.Address, server: RPCServer, card: TokenScriptCard, interpolatedFilter: String)]
    private typealias ActivityTokenObjectTokenHolder = (activity: Activity, tokenObject: Token, tokenHolder: TokenHolder)
    private typealias TokenObjectsAndXMLHandlers = [(contract: DerbyWallet.Address, server: RPCServer, xmlHandler: XMLHandler)]
    ...
    private func getActivities(forTokenContract contract: DerbyWallet.Address, server: RPCServer, card: TokenScriptCard, interpolatedFilter: String) -> [ActivityTokenObjectTokenHolder] {
        ...
        let activitiesForThisCard: [ActivityTokenObjectTokenHolder] = events.compactMap { eachEvent in
            ...
            if let holders = tokensAndTokenHolders[token.addressAndRPCServer] {
    ...
    private func generateImplicitAttributesForToken(forContract contract: DerbyWallet.Address, server: RPCServer, symbol: String) -> [String: AssetInternalValue] {
...
extension ActivitiesService.Functional {
    static func generateImplicitAttributesForCard(forContract contract: DerbyWallet.Address, server: RPCServer, event: EventActivityInstance) -> [String: AssetInternalValue] {
        var results = [String: AssetInternalValue]()
