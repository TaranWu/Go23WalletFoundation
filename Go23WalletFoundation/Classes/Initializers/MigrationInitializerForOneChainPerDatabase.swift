        ...
        config.migrationBlock = { [weak self] migration, oldSchemaVersion in
            ...
            if oldSchemaVersion < 33 {
                migration.enumerateObjects(ofType: TokenObject.className()) { oldObject, newObject in
                    ...
                    guard let address = Go23Wallet.Address(string: value) else { return }
            ...
            if oldSchemaVersion < 49 {
                ...
                migration.enumerateObjects(ofType: TokenObject.className()) { oldObject, newObject in
                    guard let oldObject = oldObject else { return }
                    guard let newObject = newObject else { return }
                    if let contract = (oldObject["contract"] as? String).flatMap({ Go23Wallet.Address(uncheckedAgainstNullAddress: $0) }), let type = (oldObject["rawType"] as? String).flatMap({ TokenType(rawValue: $0) }) {
